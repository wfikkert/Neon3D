	...
	//the maximum of addresses to be used(2600/2/21 ~= 215 Lines in total)
	#define VALUES 2600
	...
	//#define FLAGS
	#define benchmarkIsRunning_FLAG 0x01
	#define stopDrawing_FLAG 0x02
	
	//Flags acting as global bools
	OS_FLAG_GRP *flags;
	...
	int parseInteger(int amountOfDecimals, char array[4]){
	...
	void TaskShowCredits(void* pdata){
		...
		VGA_text(30,31, "13 Juni 2016 ICTESA2B Windesheim");
		int i;
		for(i = 0; i < 2000000; i++);
		resetScreen(1);
	...
	void TaskDraw3D(void* pdata)
	{
		...
		while(!stopTask3D){
			...
			if(objectID  == taskId || objectID == 0){
				...
				int err;
				OSFlagPend(flags, stopDrawing_FLAG, OS_FLAG_WAIT_SET_ANY, 0, &err);
				...
				{
					//skips index every interation by 21
				...
				//print rotation
				sprintf(onScreenZoom, "Zoom: %f", zoomscreen);
				sprintf(onScreenRotation, "Rotation X,Y,Z: %d,%d,%d", localRotationX, localRotationY, localRotationZ);
				VGA_text (21, 2, "                                          \0");
				VGA_text (15, 3, "                                          \0");
	...
	void TaskStartUp(void* pdata){
		...
		int tempi;
		for(tempi = 0; tempi <= 150; tempi++){
			*(sd_ram + (tempi+ (1 << 18))) = defaultObject[tempi];
		}
	
		//tell TaskDraw3D to start drawing
		int err;
		OSFlagPost(flags, stopDrawing_FLAG, OS_FLAG_SET, &err);
		//stopDrawing = 0;
		...
		int zRotation;
		int xRotation;
		for(zRotation = 359; zRotation >= 270; zRotation--){
	...
	void TaskBenchmark(void* pdata){
		...
		int err;
		OSFlagPost(flags, benchmarkIsRunning_FLAG, OS_FLAG_SET, &err);
		...
		*zoomscreenpointer = 0.8;
		...
		for(i =0; i <= 50; i++){
			OSTimeDly(10);
			ALT_SEM_PEND(sem_objectDrawn, 1000);
			rotation[0] = i;
			ALT_SEM_POST(sem_objectDrawn);
		}
		...
		*zoomscreenpointer = 0.8;
		...
		//Tells drawLine that it cannot put calculated values in vga buffer
		offscreen = 1;
		...
		*zoomscreenpointer = 0.8;
		...
		OSFlagPost(flags, benchmarkIsRunning_FLAG, OS_FLAG_CLR, &err);
	...
	void TaskFPSCounter(void* pdata){
		...
		while(1){
			...
			//reset all values every 5 minutes
			if(totalTimeRunning == 300){
				totalFps = 0;
				maxFps = 0;
				minFps = 10000;
				totalTimeRunning = 0;
	...
	void TaskSerialInput(void* pdata)
	{
	    ...
	    //error message handeling for flags
		int err;
		...
		//disable parts of code when upload is active
		int isUploading = 0;
	    ...
	    while (1)
	    {
			...
			OSFlagPend(flags, benchmarkIsRunning_FLAG, OS_FLAG_WAIT_CLR_ANY, 0, &err);
	
			if(dataSerial & 0x8000){
	
				//Draws progressbar for upload on screen
				if(isUploading){        //only run this if the Taskdraw3D is not drawing.
				...
				//if the FPGA receives a 'n' then the FPGA knows that there will come a new object
					...
					*zoomscreenpointer = zoom;
					...
					//stopDrawing = 1;
					printf("going to post stopDrawing flag \n");
					OSFlagPost(flags, stopDrawing_FLAG, OS_FLAG_CLR, &err);
					printf("stopDrawing flag posted \n");
			...
			if(!isUploading){
				...
				//switch 21 decreases z rotation with 1.
				...
				//switch 21 will switch between multiple objects or single object
				...
				//switch 21 increases zoom with 0.01
				...
				//switch 21 will show the credits screen
